  ...
  try {
    ...
    if (!sessionRow) {
      ...
      if (insertErr || !inserted) {
        return jsonError(
          `Could not start session: ${insertErr?.message ?? 'unknown'}`,
          500,
        );
      }
      sessionRow = inserted;
    ...
    // Confirm the candidate actually has a resume indexed.
    ...
    try {
      const result = await retrieveRelevantChunks(userMessage, profile.id);
      chunks = result.chunks;
      lowConfidence = result.lowConfidence;
    } catch (err) {
      return jsonError(
        `RAG retrieval failed: ${describeError(err)}`,
        500,
      );
    }

    const candidateName =
      profile.full_name || profile.username || 'the candidate';
    ...
    const messages = buildConversationMessages(historyMessages, userMessage);
    ...
    void (async () => {
      try {
        ...
        await service
          .from('chat_sessions')
          .update({
            question_count: savedSession.question_count + 1,
            last_active: new Date().toISOString(),
          })
    ...
    return jsonError(`Chat failed: ${describeError(err)}`, 500);
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    return jsonError(`Could not load session: ${describeError(err)}`, 500);
  }
}
