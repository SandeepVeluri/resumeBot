...
| # | Bug | Fix |
|---|---|---|
| 1 | `POST /api/chat` had no top-level `try/catch` — every downstream failure surfaced as a bare `500` with no body. The user's live report of "500 on /api/chat" was almost certainly caused by `retrieveRelevantChunks` throwing (missing `match_resume_chunks` RPC, bad `GEMINI_API_KEY`, or unindexed resume). | Wrapped the whole handler in `try/catch`; returns `{ error }` with the real error message. Added explicit checks for missing `ANTHROPIC_API_KEY` / `GEMINI_API_KEY`, and an early `404` when the candidate has no indexed resume chunks. |
